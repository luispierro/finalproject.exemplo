...
public class MusicaController {
	...
	@PostMapping("/relatorioporartistatitulo")
	public ResponseEntity<List<Musica>> getMusicaArtistaTitulo(@RequestBody Musica objeto){
	...
	public ResponseEntity<Musica> getMusicaPost(@RequestBody Musica objeto){
		Musica musica = dao.findById(objeto.getId()).orElse(null);
		if(musica==null) return ResponseEntity.status(404).build();
		return ResponseEntity.ok(musica);
